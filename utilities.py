    ...
    for i in range(len(row_ind)):
        for j in range(col):
            ...
            if len(list_of_negative) == 0:
                ...
                list_of_negative = np.random.normal(mean_value, std, 100)
                ...
                for epoch in range(25):
                    ...
                    s = torch.tensor(s)
                    s[len(list_of_negative):len(list_of_negative)+len(samples),0] = torch.tensor(samples)*std+cost[i,j]
                    x = data+s
                    x[x<0] = 0.001
                ...
                s = np.zeros([len(data[:,0]),2])
                # s = torch.tensor(s)
                s[len(list_of_negative):len(list_of_negative)+len(samples),0] = samples*std+cost[i,j]
                x = data+s
                x = x/T.item()
