...
cam_label_naive = np.zeros(len(query_label))


# # # #################################################     MAX Score (Conventional method)        ###########################################


matched_score = np.zeros(len(query_cam))
matched_label = np.zeros(len(query_cam))
matched_cam = np.zeros(len(query_cam))

itr = 0
for q in range(len(query_label)):      


    ap_tmp, CMC_tmp,gallery_index,m_score,_ = scores().score_calculation(query_gallery_c[q,:],query_gallery_rerank[q,:],query_cam[q],query_frame[q],query_label[q],gallery_cam,gallery_frame,gallery_label,time_distribution,cam = 1)
    ...
    matched_score[itr] = m_score
    matched_label[itr] = gallery_label[gallery_index]
    matched_cam[itr] = gallery_cam[gallery_index]
    itr = itr+1
...
# #       ############################################    Hungarian Method   ##############################################
    ...
    # Confidence estimation is inside the calculate_score_hungarian_without_redundancy function
    # cost_cam = adjusted confidence
    # cost_cam_naive = cosine similarity score
    ...
    if (c_n[index_max] <0) :
        ...
        for i in range(8):
            if (index_max_naive == i) and (gidx_naive[index_max_naive]<len(gallery_new_list_cam["cam_"+str(i+1)])):
                ...
                cam_label_naive[q] = i+1


    

    cost_naive[q] = c_n[index_max]
    cost_adjusted[q] = c[index_max]
...
#####################################  HUNGARIAN RESULT ##################################
# print("Neighbor matching results w/o adjusted cost",len(np.where(hung_no_redundant_label_naive==query_label)[0])/len(query_label))
# print("Neighbor matching results with adjusted cost", len(np.where(hung_no_redundant_label==query_label)[0])/len(query_label))



#####################################  Save Costs ######################################

result1 = {'cost':cost_adjusted, 'matched_label':hung_no_redundant_label,'query_label':query_label, 'matched_cam':cam_label ,'query_cam':query_cam}
result2 = {'cost':cost_naive, 'matched_label':hung_no_redundant_label_naive,'query_label':query_label,'matched_cam':cam_label_naive,'query_cam':query_cam }
result_max = {'cost':matched_score, 'matched_label':matched_label,'query_label':query_label, 'matched_cam':matched_cam ,'query_cam':query_cam}


if opt.S:

    scipy.io.savemat('./rep/'+model_name+'/'+opt.source+"_"+opt.target+'_final_result_'+opt.query_type+'_s.mat',result1)
    scipy.io.savemat('./rep/'+model_name+'/'+opt.source+"_"+opt.target+'_naive_result_'+opt.query_type+'_s.mat',result2)
else:
    scipy.io.savemat('./rep/'+model_name+'/'+opt.source+"_"+opt.target+'_final_result_'+opt.query_type+'.mat',result1)
    scipy.io.savemat('./rep/'+model_name+'/'+opt.source+"_"+opt.target+'_naive_result_'+opt.query_type+'.mat',result2)


scipy.io.savemat('./rep/'+model_name+'/'+opt.source+"_"+opt.target+'_max_result_cosinescore'+opt.query_type+'.mat',result_max)
