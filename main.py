...
#####################################  HUNGARIAN RESULT ##################################
print("Neighbor matching results with adjusted cost",len(np.where(hung_no_redundant_label_naive==query_label)[0])/len(query_label))
print("Neighbor matching results w/o adjusted cost", len(np.where(hung_no_redundant_label==query_label)[0])/len(query_label))